...
def normalize_array(array, new_min, new_max):
    ...
    If the array is already within [0, 1], normalization is skipped.
    Parameters:
    ...
    # Skip normalization if array already in range [0, 1]
    if old_min >= 0.0 and old_max <= 1.0:
        return array
    ...
    array = (array - old_min) / (old_max - old_min)
...
def generate_bordures(image, bordure, max_dim=256):
...
def incrustation(img_sim, mask_sim, mask_prop, image, value, place, NDVI=False):
    ...
    if NDVI:
        img_NDVI = (raster_img[:, :, 6] - raster_img[:, :, 2]) / (raster_img[:, :, 6] + raster_img[:, :, 2] + 1)
    ...
    img_sim = generate_bordures(img_sim, [int(mask_img.shape[0]), int(mask_img.shape[1])], max_dim=5000)
    ...
    mask_sim = generate_bordures(mask_sim, [int(mask_img.shape[0]), int(mask_img.shape[1])], max_dim=5000)
    mask_prop = generate_bordures(mask_prop, [int(mask_img.shape[0]), int(mask_img.shape[1])], max_dim=5000)
